  ...
  const setSurveyOpenClose = async (
    ...
    try {
      ...
      if (response?.status === 200) {
        ...
        // Update the survey status in the setIndividualSurveys state
        setIndividualSurveys((prevData) =>
          prevData.map((item) =>
            item.surveyTitle === surveyTitle &&
            item.surveyType === surveyType &&
            item.surveyCategory === surveyCategory
              ? {
                  ...item,
                  surveyOpenClose:
  ...
  return (
    <>
      <div className="flex flex-row items-center min-w-full">
        <h1 className="text-2xl font-semibold mb-3 mt-8">
          Market Reachout and Individual Surveys
        </h1>
        <button
        ...
        >
          <FiPlus className="mr-2" /> Create Individual Surveys
        </button>
      </div>
      <div className="w-full bg-white shadow-sm rounded-lg border border-gray-300 mb-4">
        {individualSurveys.length === 0 ? (
          <div className="text-center py-16">
            ...
            <div className="flex items-center justify-center">
              <button
                ...
                onClick={createSurvey}
              >
                <FiPlus className="mr-2" /> Create Individual Survey
              </button>
            </div>
          </div>
        ) : (
          <div className="w-full bg-white shadow-sm rounded-lg border border-gray-300">
            ...
            {individualSurveys.map((item, index) => (
              <div
                key={index}
                ...
                <div className="text-center">
                  {parseInt(item.responses) === 0 ? (
                    ...
                    <button
                      onClick={() =>
                        navigate(
                          `/analysis/${item.surveyTitle}/${item.surveyType}/individual`
                        )
                      }
...
export default IndividualSurveys;
