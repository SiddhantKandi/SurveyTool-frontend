...
const VerifyOTP = () => {
  ...
  const handleVerify = async (e) => {
    ...
    if (otp.length === 6) {
      ...
      try {
        const url = "users/verifyAdminEmailOTP";
        const response = await apiConnector("POST", url, { otp }, {}, {});
        ...
        updateToast(
          toastId,
          error?.response?.data?.message || "Error occurred while verifying the OTP.",
          "error"
        );
      } 
    } else {
